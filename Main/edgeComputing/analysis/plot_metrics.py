...
RESULTS_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../results/run01"))
os.makedirs(RESULTS_DIR, exist_ok=True)

# 1) Read all event logs
logs = glob.glob(os.path.join(RESULTS_DIR, "*_events.log"))
if not logs:
    raise SystemExit(f"No event logs found in {RESULTS_DIR}")
...
for c in clients:
    ...
    for r in rounds:
        ...
        # Increment counters and total_bytes_sent based on the final decision for the round
        if m["decision"] == "skip":
            count_skip += 1
...
for c in clients:
    ...
    plt.title(f"{c} Update Size and Decision per Round")
    plt.xlabel("Round")
    ...
    plt.tight_layout()
    out = os.path.join(RESULTS_DIR, f"{c}_update_sizes_scatter.png")
    plt.savefig(out)
    plt.close()
...
if not df.empty:
    ...
    plt.title("Decisions per client (counts)")
    plt.ylabel("count")
    plt.savefig(os.path.join(RESULTS_DIR, "decisions_per_client.png"))
    plt.close()
...
if not cdf.empty:
    ...
    plt.ylabel("skips")
    plt.title("Bytes sent vs skips (per client)")
    plt.grid(True)
    plt.savefig(os.path.join(RESULTS_DIR, "bytes_vs_skips.png"))
    plt.close()
